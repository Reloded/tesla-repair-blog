...
emoji: "❄️"
faq:
  - q: "Why is my Tesla AC blowing warm air?"
    a: "The most common cause is a software glitch — a scroll-wheel reboot fixes it about 60% of the time. If that doesn't help, check that the cabin filter isn't clogged, Climate is set to Auto, and the refrigerant level hasn't dropped from a slow leak."
  - q: "Why did my Tesla heater stop working in cold weather?"
    a: "On 2021+ heat-pump models, extreme cold can cause a high-pressure sensor fault. Tesla has issued multiple OTA fixes for this. Make sure your software is up to date, the car is connected to Wi-Fi, and try a full power-off reboot from Safety & Security > Power Off."
  - q: "How do I reset Tesla climate control?"
    a: "Hold both scroll wheels on the steering wheel for about 10 seconds until the screen goes black, then release. The touchscreen reboots in roughly 30 seconds and climate control resets. For a deeper reset, go to Controls > Safety & Security > Power Off, wait 3 minutes, then press the brake."
  - q: "Can I recharge Tesla AC refrigerant myself?"
    a: "Not recommended. Tesla uses R-1234yf refrigerant in a sealed system with specific charge weights (Model 3 uses around 550–600 g). Overcharging or mixing refrigerants will damage the compressor. Have a certified HVAC shop or Tesla Service handle refrigerant work."
  - q: "Does Tesla HVAC use a lot of battery?"
...
|---------|-------------|---------|
| AC blows warm air | Software glitch, low refrigerant | [AC Not Cooling](#ac-not-cooling) |
| Heater not warming cabin | Heat pump fault, PTC heater failure | [Heater Not Working](#heater-not-working) |
| No air from vents at all | Blower motor, cabin filter blockage | [No Airflow](#no-airflow-from-vents) |
| Weird smell from vents | Mold in evaporator, old cabin filter | [Bad Smell Fix](#bad-smell-from-vents) |
| Climate controls unresponsive | Touchscreen freeze | [Controls Frozen](#climate-controls-unresponsive) |
| "Climate Keeper Disabled" error | 12V battery or software | [Error Messages](#common-error-messages) |
...
1. **Park the car** (you can do this while driving, but parked is safer)
2. **Hold both scroll wheels** on the steering wheel simultaneously
3. **Wait 10–15 seconds** — the screen goes black
4. **Release** when the Tesla logo appears
5. Wait about 30 seconds for full reboot
...
2. Don't touch anything — no doors, no brake pedal, no screen taps
3. **Wait at least 3 full minutes**
...
**What you can't DIY:**
Refrigerant work requires specialized equipment and certification in most countries. The charge weight is specific to your model (roughly 550–600 g for Model 3) and overcharging is as bad as undercharging. Take it to Tesla or a shop with R-1234yf recovery equipment.
...
**DIY checks:**
1. Reboot the car (both methods described above)
2. Check coolant level — the PTC heater transfers heat through coolant. Low coolant = weak heat. The coolant reservoir is in the frunk (Model 3) or front trunk area
3. Look for any coolant leaks under the car (sweet smell, colored puddles)
...
**High-pressure sensor fault (cold weather):**
This hit a lot of Model 3/Y owners in winter 2021–2022. The heat pump shuts off because a sensor reads pressure incorrectly in extreme cold. Tesla has addressed this through several OTA updates.
...
Symptoms include intermittent heating — works fine for 20 minutes, then blows cold, then warm again. The thermal expansion valve can stick in cold weather.
...
**Super Manifold / Octovalve issues:**
The 2021+ heat pump system uses an "octovalve" to route coolant between the battery, cabin, and motors. A faulty valve causes erratic temperature control.
...
**Quick test:**
1. Turn fan speed to maximum
2. Put your ear near the passenger footwell area
3. You should hear a noticeable whirring sound
...
1. Tap the fan icon on the bottom bar
2. You'll see a visual of the cabin with blue dots showing where air is directed
3. Drag the dots to point at yourself
4. Make sure both driver and passenger sides are active
...
1. **Replace the cabin air filter** — this alone often solves it
2. **Run AC on max with windows open** for 10 minutes. This dries the evaporator
3. **Turn off AC 2–3 minutes before parking** but leave the fan on. This dries the evaporator surface so mold can't grow

### Deeper Clean

If the smell persists after a fresh filter:

1. Buy an **automotive AC evaporator cleaner foam** (available at any auto parts store)
2. Access the cabin filter housing
3. With the filter removed, spray the foam into the evaporator area through the filter opening
4. Let it sit for the time specified on the product (usually 15–30 minutes)
5. Run the AC on recirculation for 10 minutes
...
2. Full power-off reboot (3 minutes)
3. Check for software updates
4. Open and close all doors, then lock and unlock the car (resets some sensor states)
...
1. **Replace the cabin air filter every 12–24 months** — the single best preventive measure. A $20 filter saves you from weak airflow, bad smells, and strain on the blower motor

2. **Run AC for at least 10 minutes weekly, even in winter** — this circulates refrigerant and lubricates compressor seals. If you never run the AC in cold months, seals can dry and leak
