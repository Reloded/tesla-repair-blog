...
Tesla's lithium-ion battery cells work best between roughly 20°C and 40°C (68–104°F). Outside that range, the internal resistance of the cells increases. Higher resistance means slower charging, reduced regenerative braking, and less available range.

Preconditioning uses the battery's thermal management system — a liquid cooling and heating loop — to bring the pack into that optimal temperature window. The system circulates coolant through channels in the battery pack, heating or cooling the cells as needed.

When the battery is cold (below about 15°C), preconditioning heats it. When it's extremely hot, the system cools it down. The goal is the same either way: get the cells to a temperature where they accept and deliver energy efficiently.

**What changes when the battery is preconditioned:**

- **Charging speed** — A warm battery can accept higher charge rates at a Supercharger. Cold batteries throttle intake to prevent lithium plating on the anode, which permanently damages cells.
- **Regenerative braking** — Cold batteries limit regen. You'll see the dotted line on the regen indicator until the pack warms up. Preconditioning removes or reduces that limit.
- **Available range** — Cold cells deliver less energy. Warming the pack before driving recovers some of that lost range.
- **Acceleration** — Peak power output depends on battery temperature. A preconditioned pack delivers full torque from the start.

<div class="info-box">
<strong>Why cold batteries charge slowly:</strong> At low temperatures, lithium ions move sluggishly through the electrolyte. Pushing too much current into cold cells causes lithium to plate on the anode surface instead of intercalating properly. This is irreversible damage. Tesla's battery management system (BMS) deliberately limits charge rate to prevent this — preconditioning is the solution, not a workaround.
</div>
...
3. The car starts preconditioning automatically — you'll see a message like "Preconditioning battery for fast charging" on screen
...
<strong>Important:</strong> You must actually navigate to the charger using Tesla's built-in navigation. Using Google Maps, Waze, or Apple Maps through the car's browser won't trigger automatic preconditioning. The feature is tied to the Tesla nav system specifically.
</div>
...
<div class="info-box">
<strong>Tip:</strong> Scheduled Departure works best when the car is plugged in. Preconditioning uses energy — if the car isn't plugged in, it draws from the battery pack, which can reduce your starting range by 2–5% depending on conditions.
</div>
...
**Example — Model 3/Y at a V3 Supercharger (250 kW):**

| Battery Temp | Peak Charge Rate | Time 10→80% |
|---|---|---|
| -10°C (cold, no preconditioning) | ~30-70 kW | 60-90 min |
| 0°C (cool, partial preconditioning) | ~100-150 kW | 40-50 min |
| 25°C+ (preconditioned) | ~200-250 kW | 25-30 min |
...
It depends on the starting temperature. In mild conditions (5–15°C), the battery might precondition in 15–20 minutes. In extreme cold (-15°C or below), it can take 30–45 minutes to fully warm the pack.

### Can I precondition with a Level 1 (wall outlet) charger?

Yes, but a standard wall outlet only delivers about 1.4 kW. That's barely enough to run the climate system, let alone heat the battery. A Level 2 charger (Wall Connector or equivalent) at 7–11 kW handles preconditioning much more comfortably. Check our [Wall Connector installation guide](/posts/tesla-wall-connector-installation/) for setup details.

### Does preconditioning work in hot weather?

Yes. In extreme heat, the thermal management system cools the battery. This is less noticeable because batteries tolerate heat better than cold in the short term (though sustained heat accelerates [degradation](/posts/tesla-battery-degradation-health/)). You'll still see faster Supercharging after a long highway drive in summer because the pack is already warm — the system just keeps it from overheating.

### My car shows "Preconditioning" but I didn't turn it on — is that normal?
...
1. **Always navigate to Superchargers** — Don't just drive there. Use Tesla nav so preconditioning kicks in automatically.

2. **Plug in overnight** — Preconditioning while plugged in costs grid electricity instead of battery energy. Even a slow Level 1 connection helps.

3. **Use Scheduled Departure in winter** — Set it once, and you'll have a warm cabin and full regen every morning.

4. **Park in a garage when possible** — A garage keeps the battery 5–10°C warmer than outside. That means less energy and time needed for preconditioning.

5. **Check your battery temp** — If you have an [OBD2 scanner](/posts/tesla-obd2-scanner-diagnostics/), you can monitor exact battery temperature. The pack charges fastest above 25°C.

6. **Don't skip preconditioning for short Supercharger stops** — Even a 10-minute precondition makes a measurable difference in charge speed.

## Related Guides

- [Tesla Phantom Battery Drain Fix](/posts/tesla-phantom-battery-drain-fix/) — If your car loses range overnight
- [Tesla Range Loss Fix](/posts/tesla-range-loss-fix/) — Diagnosing and fixing range degradation
- [Tesla Battery Degradation & Health](/posts/tesla-battery-degradation-health/) — Long-term battery care
- [Tesla Winter Preparation Guide](/posts/tesla-winter-preparation-guide/) — Full cold-weather prep
- [Tesla Wall Connector Installation](/posts/tesla-wall-connector-installation/) — Home charging setup
- [Tesla Slow Charging Fix](/posts/tesla-slow-charging-fix/) — Troubleshooting slow charge rates
